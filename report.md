...
> 產生日期: 2026-04-12
> 最後校對: 2026-05-18 (依 main 分支 v1.3.1)
...
簡單來說, OpenSpec 讓你**只維護一份規格, 就能同時支援 Claude Code、Cursor、Windsurf、GitHub Copilot 等 28 種 AI 工具** (定義於 `src/core/config.ts#AI_TOOLS`)。
...
| **多工具支援** | Adapter pattern 支援 28 個 AI 工具, 每個工具有專屬格式化邏輯 |
| **變更隔離** | 每個 change 獨立存放於 `openspec/changes/` 下 |
| **雙模式 Workflow** | Core profile (輕量三步驟) 與 Full workflow (完整 artifact 追蹤) |
| **三層設定** | Global (XDG: `~/.config/openspec/` 或 `%APPDATA%\openspec\`) → Project (`openspec/config.yaml`) → Change (`.openspec.yaml`) |
...
| YAML 解析 | yaml 2.8.2 |
| 互動式 UI | @inquirer/prompts ^7.8, @inquirer/core ^10.2, chalk ^5.5, ora ^8.2 |
| 檔案搜尋 | fast-glob ^3.3 |
| 測試 | Vitest ^3.2 |
| Lint | ESLint ^9.39 + typescript-eslint ^8.50 (flat config) |
| 版本管理 | @changesets/cli ^2.27 + @changesets/changelog-github ^0.5 |
| 套件管理 | pnpm v9 (亦保留 npm lockfile) |
| 打包定義 | Nix flake (`flake.nix`) |
...
│   ├── utils/              # 共用工具函式
│   └── index.ts            # 主要匯出桶 (barrel export, 對外 library 入口)
...
│
├── cli/
│   └── index.ts                       # Commander.js 主程式, 註冊所有指令, preAction/postAction hooks (套用 --no-color)
│
...
│
├── core/                              # 核心引擎 - 所有商業邏輯
│   ├── config.ts                      # AI_TOOLS 定義 (28 個工具), OPENSPEC_MARKERS
...
> **v1.3.1 移除項目:** 先前版本曾有 `src/telemetry/` 目錄與 `posthog-node` 依賴, 現已完全移除. `src/cli/index.ts` 內仍保留空的 `preAction`/`postAction` hook 殼, 僅作為未來擴充預留位置.
...
graph TB
    ...
    subgraph "CLI 層"
        ...
        CLI -->|preAction hook| FLAGS[全域 flag<br/>--no-color]
    end
...
| **支援工具** | 28 個 (Claude, Cursor, Cline, Windsurf, Copilot, Amazon Q, Gemini, Codex, Codebuddy, Continue, Costrict, Crush, Factory, ForgeCode, iFlow, Junie, Kilo Code, Kiro, OpenCode, Pi, Qoder, Lingma, Qwen, RooCode, Trae, Antigravity, Auggie, Bob) |
...
> v1.3.1 已完全移除 PostHog 遙測機制與 `posthog-node` 依賴 (commit `76b3fb2 — docs: drop telemetry bullet from v1.3.1 changelog`). `src/cli/index.ts` 內仍保留空的 `preAction`/`postAction` hook 殼, 純粹作為未來擴充預留位置, 不會傳送任何資料.
